...
?>
	<form METHOD='POST' ACTION='NetworkMap.php'>
					...
					if ($ctype == 'web'){
						if (isset($cport)){
							if ($lvl == '1'){
								...
								$lnkstr = '<td><a href="http://'.$eqlgn.':'.$eqpswd.'@'.$gtway.':'.$cport.'">'.$fieldv[$i].'</a></td>';
							}
						}else{
							if ($lvl == '1'){
								$lnkstr = '<td><a href="/support_web/ping.php?theaddr='.$fieldv[$i].'" target="_blank">'.$fieldv[$i].'</a></td>';
							}else{
								$lnkstr = '<td><a href="http://'.$eqlgn.':'.$eqpswd.'@'.$fieldv[$i].'">'.$fieldv[$i].'</a></td>';
							}
						}
